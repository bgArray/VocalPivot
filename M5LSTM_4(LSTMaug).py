...
def preprocess_data(X, y):
    """预处理序列数据，适配LSTM输入"""
    # 新增：梅尔频谱频带切片，保留200Hz-5kHz对应的梅尔系数（假设原128维中第10-74维对应该区间）
    X_sliced = []
    for mel_seq in X:
        # mel_seq形状：(时间步, 128)，切片后变为(时间步, 64)
        sliced = mel_seq[:, 0:74]  # 具体切片范围需根据你的梅尔频谱参数调整（可参考 librosa 梅尔滤波器组计算）
        X_sliced.append(sliced)
    X = X_sliced  # 替换原X，使用切片后的特征
...
def build_lstm_model(input_shape, num_classes):
    model = Sequential([
        Masking(mask_value=0.0, input_shape=input_shape),  # 保留Masking，过滤填充的0值
        LSTM(128, return_sequences=True, dropout=0.2),  # 1层LSTM，dropout降至0.2（CPU训练更稳定）
        Dense(32, activation='relu'),  # 全连接层单元数从64→32，减少计算
        Dropout(0.3),  # dropout从0.5→0.3，避免CPU训练时数据利用率过低
