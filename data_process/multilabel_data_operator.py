...
def process_single_file(json_path: str, wav_path: str, split_by_breath: bool = True) -> List[Tuple[np.ndarray, List[List[str]]]]:
    """
...
def process_singer_data(
    ...
    if target_folders is None:
        target_folders = ["Mixed_Voice_and_Falsetto", "Breathy", "Vibrato"]
                ...
                for filename in os.listdir(group_path):
                    if filename.endswith('.json'):
                        ...
                        if os.path.exists(wav_path):
                            ...
                            if samples:
                                all_data.extend(samples)
                                print(f"处理: {singer_name}/{folder}/{song_name}/{group_name}/{filename} -> {len(samples)} 个样本")
...
if __name__ == "__main__":
    ...
    # 方式1：处理整个数据集
    # process_multilabel_dataset(
    #     data_path=DATA_PATH,
    #     split_by_breath=True,
    #     target_folders=["Mixed_Voice_and_Falsetto", "Breathy"]
    # )
    
    # 方式2：处理特定歌手的数据
    # process_singer_data(
    #     data_path=DATA_PATH,
    #     singer_name="Chinese/ZH-Alto-1",
    #     split_by_breath=True,
    #     target_folders=["Mixed_Voice_and_Falsetto", "Breathy"]
    # )
    
    # 方式3：批量处理多个歌手
    for singer in L_Singer[:3]:  # 处理前3个歌手
        print(f"\n{'='*60}")
        print(f"处理歌手: {singer}")
        print(f"{'='*60}")
        process_singer_data(
