...
def load_multilabel_data(file_path, max_samples=None):
    ...
    for item in data:
        ...
        # 处理每个时间步的标签：保留所有有效标签
        # 定义有效标签集合（数字标签和文本标签）
        valid_label_set = {"0", "1", "2", "3", "4", "5", "6", "-1", 
                          "Breathy", "Vibrato", "Glissando", "Pharyngeal",
                          "Chest", "Mixed", "Falsetto", "Head", "Normal"}
...
class LightweightMultiLabelCNNLSTM(nn.Module):
    ...
    def __init__(self, input_dim, num_labels, max_length):
        ...
        self.pointwise_conv = nn.Conv1d(in_channels=32, out_channels=64, kernel_size=1)
        self.bn2 = nn.BatchNorm1d(64)

        # LSTM层：捕捉时序特征
        # 注意：单层LSTM的dropout参数无效，只有多层LSTM才有效
        self.lstm = nn.LSTM(
            input_size=64,
            hidden_size=128,  # 减少隐藏单元数以降低参数量
            batch_first=True,
            dropout=0.0,  # 单层LSTM不支持dropout，改为0
            bidirectional=False,
        )
        # 在LSTM后添加Dropout层
        ...
        # 共享全连接层
        self.shared_fc = nn.Sequential(nn.Linear(128, 64), nn.ReLU(), nn.Dropout(0.4))
...
def main(data_path):
    ...
    print(
        "最终模型（final_multilabel_cnn_lstm_model.pth）与标签映射（label_to_idx.npy, label_to_idx.json）保存完成"
    )
